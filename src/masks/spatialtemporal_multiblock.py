...
class MaskCollator_fmri(object):
    ...
    def _sample_block_mask_p_ts(self, b_size, side='left', acceptable_regions=None):
        ...
        # Beta/gamma blocks live in the 30%-margin on each side of the temporal axis.
        # Original code hardcoded 3 and 7 for width=10; scale with self.width instead.
        side_margin = max(1, int(round(self.width * 0.3)))
        ts_mask = torch.randperm(side_margin)[:ts_m]
        if side == 'right':
            ts_mask = (self.width - side_margin) + ts_mask
